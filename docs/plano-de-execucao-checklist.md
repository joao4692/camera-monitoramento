...
6. **Testar só em pontos cruciais.** Os itens de checklist "testar ..." de cada etapa devem focar em lógica de negócio, autenticação e validações com risco real de quebrar — não em reconfirmar repetidamente coisas triviais ou já validadas (ex: uma rota simples que já funcionou). Testes automatizados (Jest) ficam reservados pra partes centrais do sistema, não para toda rota.
